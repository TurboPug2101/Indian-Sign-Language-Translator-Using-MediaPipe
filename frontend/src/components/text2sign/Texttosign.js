...
const Texttosign = () => {
  const [selectedRadio, setSelectedRadio] = useState(""); // Initial state
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        `${backendUrl}/texttosign`,
        { sentences,selectedRadio },
        { responseType: "blob" }
      );
  ...
  useEffect(() => {
    ...
    return () => {
      extractedFiles.forEach(({ content }) =>
        URL.revokeObjectURL(URL.createObjectURL(content))
      );
    };
